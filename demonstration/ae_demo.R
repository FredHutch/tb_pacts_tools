# =============================================================================
# SCRIPT NAME: AE Visualization in TB-Pacts using Dobinda's repository
# PROJECT:     SEATRAC Hackathon 
# AUTHOR:      Mark Warden
# MODIFIER:    Koshlan Mayer-Blackwell
# CREATED:     6-Jan-2026
# MODIFIED     16-Jan-2026
  # Log of major changes (16-Jan-2026) - KMB:
  # 1. Configured to use reference paths to data and metadata.
  # 2. Uses a package structure to load auxillary files for data cleaning, variable_names, anc ctgov cocde
  # 3. Allows user to provide one or more study identifiers 
  # 4. Makes explicit  code for data loading  (simpler for new users of R) and doesn't require being in a working directory 
  # 5. Added general yurpose tool for getting Variable,Label for any dataset
  # 6. Added plot functions to save these to a figure_save_folder
...
#k# PRELIMINARY INFORMATION
  #K# DATA FOLDER PATHS THAT WILL CHANGE
...
#K# LOAD CODE MODULES -This script relies on functions that are defined in separate modules /demonstration/utils/
  #K# OPTION 1: SOURCE FILES DIRECTLY
#source('/Volumes/fh/fast/gilbert_p/fg_data/TB-PACTS/gitrepos/tb_pacts_tools/R/data_cleaning_functions.R')
#source('/Volumes/fh/fast/gilbert_p/fg_data/TB-PACTS/gitrepos/tb_pacts_tools/R/ctgov_functions.R') 
  #K# PERFERRED OPTION 2: DEVTOOLS LAODS FUNCTION PACKAGE
...
ae = readRDS(file.path(tb_pacts_rds, 'ae.rds')) %>% 
  filter(STUDYID %in% study_identifiers )
dm = readRDS(file.path(tb_pacts_rds, 'dm.rds')) %>% 
  filter(STUDYID %in% study_identifiers )
ta = readRDS(file.path(tb_pacts_rds, 'ta.rds')) %>% 
  filter(STUDYID %in% study_identifiers ) 
tv = readRDS(file.path(tb_pacts_rds, 'tv.rds')) %>% 
  filter(STUDYID %in% study_identifiers ) 
ds = readRDS(file.path(tb_pacts_rds, 'ds.rds')) %>% 
  filter(STUDYID %in% study_identifiers ) 
sv = readRDS(file.path(tb_pacts_rds, 'sv.rds')) %>% 
  filter(STUDYID %in% study_identifiers ) 
...
ta_labels <- get_cdisc_labels("ta", metadata_folder = metadata_folder)
tv_labels <- get_cdisc_labels("tv", metadata_folder = metadata_folder)
ds_labels <- get_cdisc_labels("ds", metadata_folder = metadata_folder)
sv_labels <- get_cdisc_labels("sv", metadata_folder = metadata_folder)
...
ta  = rm_cols(ta,tolower=TRUE) 
tv  = rm_cols(tv,tolower=TRUE) 
ds  = rm_cols(ds,tolower=TRUE) 
sv  = rm_cols(sv,tolower=TRUE) 

summarytools::label(ae)<-"Adverse event tracking"
summarytools::label(dm)<-"Age, Gender, Race, Trial Arm data"
summarytools::label(ta)<-"Description of trial arms of each study"
summarytools::label(tv)<-"List of scheduled visits for each study"
summarytools::label(ds)<-"Data on protocol milestones  and study completion or early withdrawal status"
summarytools::label(sv)<-"Table of subject visits. Some unscheduled visits are not in this table. This table should be used as a reference only"
...
# NOW YOU SHOULD BE ABLE TO SEE WHAT VARIABLE CODES ARE ARE:
# USE SUMMARY TOOLS TO EXAMINE DATA 
ae %>% dfSummary() %>% stview()
dm %>% dfSummary() %>% stview()
ta %>% dfSummary() %>% stview()
tv %>% dfSummary() %>% stview()
...
###### <<<< ---------------------------->>>>> ####
###### HERE IS PRIMARY FORMATTIGN SCRIPT #########
